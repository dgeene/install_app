...
def main(ios_deploy_path, adb_path, app_path, file_ext) -> None:
    if file_ext[1] == '.ipa':
        devices = list_ios_devices(ios_deploy_path)
        choice = handle_device_selection(devices)
        install_ios_device(ios_deploy_path, choice, app_path)
    elif file_ext[1] == '.apk':
        devices = list_android_devices(adb_path)
        choice = handle_device_selection(devices)
        install_android_device(adb_path, choice, app_path)
    else:
...
if __name__ == '__main__':
    ...
    main(ios_deploy_path, adb_path, app_path, file_ext)